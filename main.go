...
func main() {
	bot, err := tgbotapi.NewBotAPI("1760751596:AAFyiuBK1lma4dIxWwPzVlLmHulbULdImRI")
	if err != nil {
		log.Panic(err)
	}
	...
	for update := range updates {
		...
		case "ADD":
			if len(command) != 3 {
				bot.Send(tgbotapi.NewMessage(update.Message.Chat.ID, "incorrect syntax"))
				return
			}
		...
		case "DEL":
			if len(command) != 2 {
				bot.Send(tgbotapi.NewMessage(update.Message.Chat.ID, "the syntax of the command is incorrect"))
			}
